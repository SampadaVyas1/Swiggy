...
const useOverlayPosition = (
  ...
  const getPosition = () => {
    ...
    const spaceBelow = window.innerHeight - (targetRef.current.offsetTop + targetRef.current.offsetHeight);
    const spaceLeft = targetRef.current.offsetLeft;
    const spaceRight = window.innerWidth - (targetRef.current.offsetLeft + targetRef.current.offsetWidth);
  ...
  const getCombinedPosition = (availablePositions: string | string[]) => {
    const getAlignment = (position: overlayPositions | undefined, alignmentOptions: overlayPositions[]) => {
    ...
    const verticalAlignment = getAlignment(defaultPosition, [OverlayPosition.Top, OverlayPosition.Bottom]);
    const horizontalAlignment = getAlignment(defaultPosition, [OverlayPosition.Left, OverlayPosition.Right]);
  ...
  const handleOverlayPosition = () => {
    ...
    if (defaultPosition && newPositions.includes(defaultPosition)) {
      ...
      const positionMap: { [key: string]: overlayPositions } = {
